...
def predict_future(model, df_requests, df_resources, last_n_timestamps=10, prediction_steps=5):
    ...
    # Save node embeddings (features) to CSV
    node_embeddings_df = pd.DataFrame(
        features_tensor.numpy(),
        index=[inverse_mapping[i] for i in range(len(features))],
        columns=[f'feature_{i}' for i in range(features_tensor.shape[1])]
    )
    # Create a list to store all embeddings
    all_node_embeddings = []
    
    # Add timestamp information and node information
    for timestamp in latest_timestamps:
        timestamp_embeddings = node_embeddings_df.copy()
        timestamp_embeddings['timestamp'] = timestamp
        all_node_embeddings.append(timestamp_embeddings)
    
    # Concatenate all embeddings and save
    combined_embeddings = pd.concat(all_node_embeddings, axis=0)
    combined_embeddings.index.name = 'service'
    combined_embeddings.to_csv('./embeddings/node_embeddings.csv')

    # Save edge information to CSV
    edge_data = {
        'source': [inverse_mapping[idx] for idx in edge_index_tensor[0].numpy()],
        'target': [inverse_mapping[idx] for idx in edge_index_tensor[1].numpy()]
    }
    edge_embeddings_df = pd.DataFrame(edge_data)
    edge_embeddings_df.to_csv('./embeddings/edge_embeddings.csv', index=False)
